...
describe DplaApiResponseBuilder do
  ...
  describe '#curated_memberships' do
    def stub_docs(docs, count: docs.size)
      allow(response).to receive(:code).and_return(200)
      allow(response).to receive(:body)
        .and_return({ 'docs' => docs, 'count' => count }.to_json)
    end
    ...
    it 'maps item ids to slugs, handling both array and bare-string fields' do
      ...
      expect(builder.curated_memberships(:exhibitions, 'HathiTrust'))
    ...
    it 'asks for the institution rather than for individual items' do
      stub_docs([])
      expect(described_class).to receive(:get).with(
        '/items',
        hash_including(
      ...
      builder.curated_memberships(:primary_source_sets, 'HathiTrust', 'University of Michigan')
    end

    it 'stops after one request when the first page holds everything' do
      stub_docs([{ 'id' => 'aaa', 'exhibitions' => 'activism' }])
      expect(described_class).to receive(:get).exactly(1).times.and_return(response)
      builder.curated_memberships(:exhibitions, 'HathiTrust')
    end

    it 'pages until it has every item' do
      stub_docs([{ 'id' => 'aaa', 'exhibitions' => 'activism' }], count: 99)
      expect(described_class).to receive(:get)
        .exactly(DplaApiResponseBuilder::CURATED_MAX_PAGES).times.and_return(response)
      builder.curated_memberships(:exhibitions, 'HathiTrust')
    end
